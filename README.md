...
local Window = Rayfield:CreateWindow({
	...
	KeySettings = {
		...
		Key = {[1] = "ßƘÆƁŒ№¥ŁØŁ♠︎◆"} -- 이 필드는 비워두고 직접 검증 로직 구현
	}
})
---------------------- 기본기능 함수 모음 -----------------------
...
local Button_Lang_All_Kill = Tab3:CreateButton({
	Name = "킬올",
	Callback = function()
		showMessage("Alll Kill")
		cuffSpecificPlayer()-- 함수 채우기
	end
})
...
local Button_tebi_jak_give = Tab3:CreateButton({
	Name = "모든 총 가져오기",
	Callback = function() Toggle_Tebi_tool_giver_f() 
...
local Button_tebi_jak_vote = Tab3:CreateButton({
	...
	Callback = function() Toggle_Tebi_vote_f() 
		showMessage("투표창 띄우기")
	end-- 함수 채우기
})
...
local Button_Sak_ALL_Cuff = Tab3:CreateButton({
	Name = "ALL Cuff",
	Callback = function() cuffAllPlayersa() 
		showMessage("ALL Cuff")
	end-- 함수 채우기
})
